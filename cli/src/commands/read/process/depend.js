    ...
    if (processResult.length > 0) {
      try {
        var mapResult = await knex
          ...
          .join(
            "process_artefact_rel AS source",
            "current.source_artefact_id",
            "source.target_artefact_id"
          )
          .join("process", "source.process_id", "process.process_id")
...
CustomCommand.description = "retrieve source processes this process depends on";
