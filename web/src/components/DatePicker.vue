...
export default {
  data() {
    return { apiStore: useApiStore(), dateStore: useDateStore(), datePicker: null }
  },
  created() {
    this.setAuditDates()
  },
  ...
  methods: {
    ...
    setEventListeners() {
      ...
      startEl.addEventListener(
        'changeDate',
        (e) => (this.dateStore.startDate = moment(e.detail.date).format('YYYY-MM-DD'))
      )
      endEl.addEventListener(
        'changeDate',
        (e) => (this.dateStore.endDate = moment(e.detail.date).format('YYYY-MM-DD'))
      )
    },
    setAuditDates() {
      if (!this.dateStore.startDate || !this.dateStore.endDate) {
        ...
        axios.get(url.href).then((response) => {
          ...
          this.datePicker.setDates(this.dateStore.startDate, this.dateStore.endDate)
        })
      }
    }
  },
