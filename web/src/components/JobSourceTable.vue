...
export default {
  data() {
    return { apiStore: useApiStore(), rawJobSourceData: null };
  },
  created() {
    this.getJobSourceData();
  },
  computed: {},
  methods: {
    getJobSourceData() {
      const url = new URL(this.apiStore.jobSource);
      url.searchParams.append("name", this.jobName);
      axios
        .get(url.href)
        .then((response) => (this.rawJobSourceData = response.data));
    },
    formatDate(date) {
      if (date) {
