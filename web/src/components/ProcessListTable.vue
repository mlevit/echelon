...
export default {
  ...
  computed: {
    processData() {
      if (this.rawProcessData) {
        // Filter the data based on the search term
        let data = this.rawProcessData.filter((item) => {
          if (this.typeTerm === '') {
            ...
            return (
              ...
              item.type.toLowerCase() === this.typeTerm.toLowerCase()
            )
          }
        })
    ...
    dataLength() {
      if (this.rawProcessData) {
        return this.searchTerm ? this.processData.length : this.rawProcessData.length
      } else {
        return 0
      }
    },
  ...
  methods: {
    getProcessData() {
      ...
      axios.get(url.href).then((response) => (this.rawProcessData = response.data))
    },
    getProcessTypeData() {
      const url = new URL(this.apiStore.process)
      url.searchParams.append('jq', '[.[] | .type] | unique | sort')
      axios.get(url.href).then((response) => (this.rawProcessTypeData = response.data))
    },
    formatDate(date) {
      if (date) {
  ...
  <div class="overflow-x-autosm:rounded-lg relative w-full">
    <div class="flex items-center justify-between pb-4">
      <div>
        <button
          ...
          class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:hover:border-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-700"
          type="button"
        >
        ...
        >
          ...
          >
            ...
            <li>
              <div
                class="flex items-center rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-gray-600"
                v-for="record in rawProcessTypeData"
                :key="record"
              >
                <input
    ...
    <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400"
        ...
        >
        of
        <span class="font-semibold text-gray-900 dark:text-white" v-if="rawProcessData">{{
          dataLength
        }}</span></span
      >
