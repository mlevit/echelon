...
export default {
  ...
  methods: {
    getAuditData() {
      const url = new URL(this.apiStore.processAuditLog);
      url.searchParams.append("id", this.auditId);
      axios
        .get(url.href)
        .then((response) => (this.rawAuditLogData = response.data));
    ...
    createInterval() {
      if (this.auditStatus === "running") {
        this.liveIntervalId = window.setInterval(this.getAuditData, 5000);
      } else {
        this.clearInterval();
      }
    },
    copyData() {
      navigator.clipboard.writeText(
        JSON.stringify(this.rawAuditLogData, null, 2)
      );
    },
    formatDate(date) {
      if (date) {
  ...
  <div class="grid w-full gap-y-4 dark:bg-gray-900">
    ...
    <div>
      <div
        ...
        v-if="!_.isEmpty(rawAuditLogData)"
      >
        <table
        ...
        >
          ...
          <tbody>
            <tr
              ...
              v-for="record in rawAuditLogData"
              :key="record.log_id"
            >
