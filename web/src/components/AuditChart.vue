...
export default {
  ...
  mounted() {
    ...
    this.dashboardChart = echarts.init(document.getElementById('dashboard-chart'))
  },
  computed: {
    colorMode() {
    ...
    hiddenClass() {
      return _.isEmpty(this.rawAuditData) ? 'hidden' : ''
    }
  },
  methods: {
    getAuditData() {
  ...
  watch: {
    rawAuditData() {
      ...
      */
      const xAxis = new Set(
        this.rawAuditData.flatMap((obj) => obj[Object.keys(obj)[0]].map((item) => item.date))
      )

      /*
      Fill in missing dates in each array to have zero values
      */
      this.rawAuditData.forEach((obj) => {
      ...
      */
      const series = Object.entries(
        this.rawAuditData.reduce((acc, obj) => {
          const status = Object.keys(obj)[0]
          const statusArray = obj[status]
