...
export default {
  ...
  methods: {
    artefactLink(artefactId) {
      router.push(`/data/${artefactId}`)
    },
    getArtefactData() {
      if (this.artefactName) {
        const url = new URL(this.apiStore.artefactLineage)
    ...
    getLineageGraph() {
      var lineage = []
      for (let record of this.rawArtefactData) {
        lineage.push({
          id: record.target_artefact_id,
          text: record.target_artefact_name,
          editable: true,
          edgeType: 'round'
        })

        lineage.push({
          id: record.source_artefact_id,
          text: record.source_artefact_name,
          link: '-->',
          next: [record.target_artefact_id],
          editable: true,
          edgeType: 'round'
        })
      }
