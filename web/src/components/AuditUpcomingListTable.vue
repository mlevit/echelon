...
export default {
  ...
  created() {
    ...
    this.dateStore.$subscribe(this.getProcessRunData)
  },
  mounted() {
  ...
  computed: {
    processRunData() {
      if (this.rawProcessRunData) {
        let data = this.rawProcessRunData
    ...
    dataLength() {
      if (this.rawProcessRunData) {
        if (this.searchTerm || this.statusTerm) {
          return this.processRunData.length
        } else {
          return this.rawProcessRunData.length
        }
      } else {
        return 0
      }
  ...
  watch: { rawProcessRunData() {} }
}
</script>

<template>
  <div v-if="!_.isEmpty(rawProcessRunData)" class="overflow-x-autosm:rounded-lg relative w-full">
    <div>
    ...
    <div class="overflow-x-auto border border-gray-300 dark:border-gray-600 sm:rounded-lg">
      <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr
            v-for="record in processRunData"
    ...
    <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
        ...
        <span
          ...
          v-if="!_.isEmpty(rawProcessRunData)"
        >
          {{ dataLength }}
        </span>
      </span>
